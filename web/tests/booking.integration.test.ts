...
if (api !== "http://127.0.0.1:56321")
  throw new Error("Integration tests require isolated local Murmur.");
    ...
    if (url.pathname.endsWith("/freeBusy"))
      return Response.json({
        calendars: Object.fromEntries(
          body.items.map((i: { id: string }) => [i.id, { busy: googleBusy }]),
        ),
      });
    if (method === "POST") {
      if (failInsert)
        return Response.json({ error: "backendError" }, { status: 500 });
      return Response.json({
        ...body,
        id: `event-${calls.length}`,
...
const day = new Date(
  Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() + 3),
);
const at = (h: number, m = 0) =>
  new Date(day.getTime() + (h * 60 + m) * 60_000);
const iso = (h: number, m = 0) => at(h, m).toISOString();
const open = async () =>
  (await slotsFor(handle, "intro", at(0), at(24), "UTC")).map((d) =>
    d.toISOString(),
  );
...
async function createUser() {
  const email = `booking-${randomUUID()}@example.invalid`,
...
describe("booking links against the host's own calendar", () => {
  it("offers only times that are free on every calendar, the Mac and existing bookings", async () => {
    ...
    must(
      await admin
        ...
        .upsert({
          user_id: userID,
          blocks: [{ start: iso(18), end: iso(19) }],
        }),
    );
    const slots = await open();
    expect(slots).toEqual(expect.arrayContaining([iso(9), iso(12), iso(20)]));
    for (const taken of [iso(10), iso(10, 30), iso(18), iso(18, 30)])
      expect(slots).not.toContain(taken);
  ...
  it("books on the host's calendar so Google sends the invitation and Meet link", async () => {
    ...
    expect(
      first.manage_url.startsWith(`${site}/book/manage/${first.id}#`),
    ).toBe(true);
    const insert = calls
      .slice(before)
      .find(
        (c) =>
          c.method === "POST" &&
          c.url.pathname.endsWith("/calendars/primary/events"),
      )!;
    ...
    expect(insert.body!.attendees).toEqual([
      { email: guest.email, displayName: guest.name },
    ]);
    expect(
      insert.body!.conferenceData.createRequest.conferenceSolutionKey.type,
    ).toBe("hangoutsMeet");
    ...
    const row = must(
      await admin.from("bookings").select("*").eq("id", first.id).single(),
    );
    expect(row).toMatchObject({
      ...
      answers: [
        {
          question: "What would you like to cover?",
          answer: "Pricing for a team of 12",
        },
      ],
    });
  ...
  it("refuses a missing required answer and a time that is already booked", async () => {
    await expect(
      createBooking(handle, "intro", { ...guest, start: iso(13), answers: {} }),
    ).rejects.toMatchObject({ status: 400 });
    await expect(
      createBooking(handle, "intro", {
        ...guest,
  ...
  it("keeps nothing when Google refuses the event", async () => {
    failInsert = true;
    await expect(
      createBooking(handle, "intro", { ...guest, start: iso(14) }),
  ...
  it("shows a guest only their own booking, and only with its private link", async () => {
    ...
    await expect(
      guestView(first.id, randomBytes(32).toString("base64url")),
  ...
  it("moves a booking only to a free time, and Google tells the guest", async () => {
    ...
    await expect(
      rescheduleBooking(first.id, token, iso(15), "UTC"),
    ).rejects.toMatchObject({ status: 409 });
    googleBusy = [];
    const before = calls.length;
    expect(await rescheduleBooking(first.id, token, iso(15), "UTC")).toEqual({
      starts_at: iso(15),
      ends_at: iso(15, 30),
    });
  ...
  it("cancels from the guest's link or by the host, and frees the time", async () => {
    ...
    expect(
      await cancelBooking(
        first.id,
        { token: tokenOf(first.manage_url) },
        "Something came up",
      ),
    ...
    expect(
      (await guestView(first.id, tokenOf(first.manage_url))).meeting_url,
    ).toBeNull();
    const again = await createBooking(handle, "intro", {
      ...guest,
      start: iso(15),
    });
    await expect(
      cancelBooking(again.id, { hostID: users[1] }),
    ).rejects.toMatchObject({ status: 404 });
    expect(await cancelBooking(again.id, { hostID: userID })).toEqual({
      status: "cancelled",
    });
  });
  ...
  it("attaches the guest's booking details to the host's agenda", async () => {
    const booked = await createBooking(handle, "intro", {
      ...guest,
      start: iso(20),
    });
    const { provider_event_id } = must(
      await admin
        .from("bookings")
    ...
    expect(
      view.events.find((e) => e.id === provider_event_id)?.booking,
    ).toMatchObject({
      id: booked.id,
  ...
  it("suggests free time to connected AI apps without holding it", async () => {
    const count = async () =>
      must(await admin.from("bookings").select("id").eq("user_id", userID))
        .length;
    const before = await count();
    const result = await freeTimes(userID, {
      days: 7,
      durationMinutes: 30,
      timeZone: "UTC",
    });
  ...
  it("limits public requests per address without storing the address", async () => {
    ...
    await rateLimit(from("203.0.113.7"), bucket, 2, 3600);
    await expect(
      rateLimit(from("203.0.113.7"), bucket, 2, 3600),
...
describe("meeting-type availability profiles", () => {
  it("uses a reusable schedule, follows edits, and preserves the public identity", async () => {
    const { effectiveProfile } = await import(
      "../src/lib/scheduling/schedules"
    );
    const profile = must(
      await admin
    ...
    const type = must(
      await admin.from("event_types").select("*").eq("id", typeID).single(),
    );
    const schedule = must(
      await admin
    ...
    const saved = { ...type, availability_schedule_id: schedule.id };
    const result = await effectiveProfile(profile, saved);
    expect(result.handle).toBe(profile.handle);
    expect(result.weekly_hours).toEqual(schedule.weekly_hours);
    expect(result.time_zone).toBe("Australia/Sydney");
    must(
      await admin
        .from("availability_schedules")
        .update({ weekly_hours: [{ days: [2], start: "14:00", end: "15:00" }] })
        .eq("id", schedule.id),
    );
    expect((await effectiveProfile(profile, saved)).weekly_hours).toEqual([
      { days: [2], start: "14:00", end: "15:00" },
    ]);
    const custom = {
      ...type,
      availability_override: {
        ...
        weekly_hours: [],
        date_overrides: [
          { date: "2026-12-01", ranges: [{ start: "09:00", end: "10:00" }] },
        ],
      },
    };
    expect((await effectiveProfile(profile, custom)).date_overrides).toEqual(
      custom.availability_override.date_overrides,
    );
    await expect(
      effectiveProfile({ ...profile, user_id: users[1] }, saved),
    ).rejects.toThrow(/availability needs attention/);
  });
});
