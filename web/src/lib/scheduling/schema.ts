...
const time = z.string().regex(/^(([01]\d|2[0-3]):[0-5]\d|24:00)$/, "Use a time such as 09:00.");
const range = z
  .object({ start: time, end: time })
  .refine((r) => r.start < r.end, "Each block of hours must end after it starts.");

export const RESERVED_HANDLES = ["manage", "api", "new"];
export const TEMPLATES = ["meeting", "oneOnOne", "standup", "interview"] as const;
export const LOCATIONS = ["google_meet", "video_link", "in_person", "phone"] as const;

export const timeZoneSchema = z.string().max(64).refine(isTimeZone, "Choose a valid time zone.");
export const handleSchema = z
  ...
  .refine((h) => !RESERVED_HANDLES.includes(h), "That link name is reserved. Choose another.");
export const slugSchema = z
  .string()
  .trim()
  .toLowerCase()
  .regex(/^[a-z0-9]([a-z0-9-]{0,48}[a-z0-9])?$/, "Use lowercase letters, numbers or hyphens.");
...
export const dateOverridesSchema = z
  .array(z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), ranges: z.array(range).max(10) }))
  .max(400);

export const profileSchema = z.object({
  handle: handleSchema,
  display_name: z.string().trim().min(1, "Add the name guests will see.").max(120),
...
export const eventTypeSchema = z
  .object({
    ...
    slot_interval_minutes: z.number().int().min(5).max(480).nullable().default(null),
  ...
  .superRefine((value, ctx) => {
    if (value.location_kind === "video_link") {
      try {
        if (new URL(value.location_detail || "").protocol !== "https:") throw new Error();
      } catch {
        ctx.addIssue({ code: "custom", path: ["location_detail"], message: "Add your meeting room link, starting with https://." });
      }
    }
    if (value.location_kind === "in_person" && !value.location_detail)
      ctx.addIssue({ code: "custom", path: ["location_detail"], message: "Add the address where you'll meet." });
    if (new Set(value.questions.map((q) => q.id)).size !== value.questions.length)
      ctx.addIssue({ code: "custom", path: ["questions"], message: "Each question needs its own ID." });
  });

export const bookingRequestSchema = z.object({
  start: z.iso.datetime({ offset: true }),
  name: z.string().trim().min(1, "Add your name.").max(120),
  email: z.email("Add an email address where you can receive the invitation.").max(320),
  time_zone: timeZoneSchema,
...
export type EventType = z.infer<typeof eventTypeSchema> & { id: string; user_id: string };
