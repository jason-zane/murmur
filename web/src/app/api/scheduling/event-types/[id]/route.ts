...
async function id(params: Promise<{ id: string }>) {
  ...
  if (!z.uuid().safeParse(id).success) throw new HttpError(404, "This meeting type doesn’t exist.");
  return id;
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { client } = await requireEditor(request);
    const { data, error } = await client.from("event_types").delete().eq("id", await id(params)).select("id");
    if (error) throw error;
    if (!data?.length) throw new HttpError(404, "This meeting type doesn’t exist.");
