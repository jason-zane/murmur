...
actor AppleSpeechEngine: TranscriptionEngine {
    ...
    /// Whether any audio reached the analyzer this session. Finalizing an analysis that
    /// never received input does not return promptly — it is the mechanism behind a stop
    /// that sits on "Transcribing…" indefinitely — so with no input we cancel instead.
    ...
    func finish() async {
        ...
        guard didReceiveAudio else {
            // Nothing to transcribe. `finalizeAndFinishThroughEndOfInput` on an empty
